...
include _ROOT_PATH.'/app/security/check_kredyt.php';
...
function process(&$amount,&$period,&$percent,&$messages,&$result){
	...
	if ($role=='admin')
        ...
        {
           $messages [] = 'Nie pożyczamy studentom';
        }
	else if ($role =='user')
        {
            $messages [] = 'Aplikacja jest w fazie testów tylko administrator może wyliczać ratę';
        }
...
if ( validate($amount,$period,$percent,$messages) ) { // gdy brak błędów
